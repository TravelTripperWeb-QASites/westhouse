...
(function($){
  ...
  $.fn.rezGallery = function(options){
   ...
   var defaults = {
   	   ...
   	   customHTML : '<h5> Special Offer</h5>', // extra html code abve the title
        ...
        success: function (response) {
		    ...
		    $(galleryelemts).each(function(){
		    	...
		    	if(item < galleryLength && item < response.special_rates.length){
                    ...
                    $this.attr('data-sub-html',options.customHTML+'<h4>'+response.special_rates[item].rate_plan_name+'</h4> '+rez_detail+' <p><a class="'+options.buttonClass+'"  href="'+offercode+'">'+options.buttonText+'</a></p>');
